...
class RsaPrivateKey(PrivateKey):
  ...
  # em - encoded message
  def __Decode(self, encoded_message, label=b""):
    ...
    if diff_len > 1:
      ...
      encoded_message = util.RepeatByte(0x00,(diff_len - 1)) + encoded_message
    ...
    delimited_message = util.TrimBytes(datablock[util.HLEN:])
    if delimited_message[0] != b'\x01'[0]:
  ...
  def Sign(self, msg):
    ...
    bigint = util.BigIntToBytes(self.key.sign(emsa_encoded, None)[0])
    return util.PadBytes(bigint, (self.size // 8) - len(bigint))   
