 ...
 * 
 * From http://java.sun.com/javase/6/docs/technotes/guides/security/StandardNames.html:
 * "The ECDSA signature algorithms as defined in ANSI X9.62." This means that an ECDSA 
 * signature is encoded as SEQUENCE { INTEGER, INTEGER } in ASN.1.
 * 
...
//			seq.addElement(new INTEGER(EcCore.fieldElemToBytes(s, params)));
			...
			byte[] tmp = new byte[2+(((ECFieldFp)params.getCurve().getField()).getFieldSize()+7)/8];
			tmp[0] = 0x02;
...
//			seqtemp.addElement( INTEGER.getTemplate() );            
//			SEQUENCE seq = (SEQUENCE)seqtemp.decode(new ByteArrayInputStream(sigBytes));
...
//			BigInteger s = (BigInteger)seq.elementAt(1);
			...
			// verification. In this case they should have used the BigInteger(1,byte[])
