...
/**
 * This class implements the basic EC operations such as point addition and doubling and point
 * multiplication. Only NSA Suite B / NIST curves are supported.
 *
 * TODO(martclau): Add (more) comments, Performance optimizations, Cleanup ASN.1 code, possibly replace
 * with own impl 
 *
 * References:
 *
 * [1] Software Implementation of the NIST Elliptic Curves Over Prime Fields, M. Brown et al.
 * [2] Efficient elliptic curve exponentiation using mixed coordinates, H. Cohen et al. 
 * [3] SEC 1: Elliptic Curve Cryptography. 
 * [4] Guide to Elliptic Curve Cryptography, D. Hankerson et al., Springer.
 *
...
public final class EcCore extends Provider {
  ...
  public EcCore() {
    ...
    AccessController.doPrivileged(new PrivilegedAction<Object>() {
      ...
      public Object run() {
        ...
        put("Signature.SHA256withECDSA", "org.keyczar.jce.EcSignatureImpl$SHA256");
        put("Signature.SHA384withECDSA", "org.keyczar.jce.EcSignatureImpl$SHA384");
        put("Signature.SHA512withECDSA", "org.keyczar.jce.EcSignatureImpl$SHA512");
  ...
  private static final ECParameterSpec P192 = new ECParameterSpec(new EllipticCurve(
      new ECFieldFp(
  ...
  private static final ECParameterSpec P224 = new ECParameterSpec(new EllipticCurve(
      new ECFieldFp(
      ...
      new ECPoint(
          ...
          new BigInteger("FFFFFFFFFFFFFFFFFFFFFFFFFFFF16A2E0B8F03E13DD29455C5C2A3D", 16), 1);

  private static final
      ECParameterSpec P256 = new ECParameterSpec(new EllipticCurve(new ECFieldFp(new BigInteger(
  ...
  private static final
      ECParameterSpec P384 = new ECParameterSpec(new EllipticCurve(new ECFieldFp(new BigInteger(
  ...
  private static final
      ECParameterSpec P521 = new ECParameterSpec(new EllipticCurve(new ECFieldFp(new BigInteger(
  ...
  private static Map<ECParameterSpec, String> friendlyNameMap =
      new HashMap<ECParameterSpec, String>();
  ...
  public static String getOID(ECParameterSpec params) {
    String oid;
    if ((oid = paramsMap.get(params)) != null) {
      return oid;
    }
  ...
  public static String getFriendlyName(ECParameterSpec params) {
    String name;
    if ((name = friendlyNameMap.get(params)) != null) {
      return name;
    }
    throw new IllegalArgumentException("Unsupport EC parameters");
  }
  ...
  private static BigInteger[] doublePointA(BigInteger[] P, ECParameterSpec params) {
    ...
    BigInteger d = (P[0].pow(2).multiply(THREE).add(a)).multiply(P[1].shiftLeft(1).modInverse(p));
  ...
  private static BigInteger[] addPointsA(BigInteger[] P1, BigInteger[] P2, ECParameterSpec params) {
    final BigInteger p = ((ECFieldFp) params.getCurve().getField()).getP();
    ...
    BigInteger d = (P2[1].subtract(P1[1])).multiply((P2[0].subtract(P1[0])).modInverse(p));
    BigInteger[] R = new BigInteger[2];
  ...
  private static BigInteger[] multiplyPointA(BigInteger[] P, BigInteger k, ECParameterSpec params) {
  ...
  private static BigInteger[] doublePointJ(BigInteger[] P, ECParameterSpec params) {
    final BigInteger p = ((ECFieldFp) params.getCurve().getField()).getP();
    BigInteger A, B, C, D;
    ...
    C = THREE.multiply(P[0].subtract(P[2].pow(2))).multiply(P[0].add(P[2].pow(2))).mod(p);
    D = C.pow(2).subtract(A.add(A)).mod(p);

    return new BigInteger[] {
        D, C.multiply(A.subtract(D)).subtract(B).mod(p), TWO.multiply(P[1]).multiply(P[2]).mod(p)};
  }

  private static BigInteger[] addPointsJA(
      BigInteger[] P1, BigInteger[] P2, ECParameterSpec params) {
    final BigInteger p = ((ECFieldFp) params.getCurve().getField()).getP();
    BigInteger A, B, C, D;
    BigInteger X3;
    ...
    X3 = D.pow(2).subtract(C.pow(3).add(TWO.multiply(P1[0]).multiply(C.pow(2)))).mod(p);
    return new BigInteger[] {X3,
        D.multiply(P1[0].multiply(C.pow(2)).subtract(X3)).subtract(P1[1].multiply(C.pow(3))).mod(p),
        P1[2].multiply(C).mod(p)};
  }

  // Binary NAF method for point multiplication
  static BigInteger[] multiplyPoint(BigInteger[] P, BigInteger k, ECParameterSpec params) {
  ...
  static BigInteger[] multiplyPoints(
      BigInteger[] P, BigInteger k, BigInteger[] Q, BigInteger l, ECParameterSpec params) {
  ...
  static int fieldElemToBytes(BigInteger a, ECParameterSpec params, byte[] data, int off) {
    int len = (((ECFieldFp) params.getCurve().getField()).getP().bitLength() + 7) / 8;
    byte[] bytes = a.toByteArray();
    if (len < bytes.length) {
  ...
  static ECPoint bytesToECPoint(byte[] bytes, ECParameterSpec params) {
    switch (bytes[0]) {
      case 0x00: // point at inf
        throw new IllegalArgumentException("Point at infinity is not a valid argument");
      case 0x02: // point compression
      case 0x03:
        throw new UnsupportedOperationException("Point compression is not supported");
      case 0x04:
        final BigInteger p = ((ECFieldFp) params.getCurve().getField()).getP();
  ...
  public static void main(String[] args) throws Exception {
    ...
    // P = kG
    BigInteger k =
        new BigInteger("a9231e0d113abdacd3bb5edb24124fbef6f562c5f90b835670f5e48f775019f2", 16);
    BigInteger[] P = new BigInteger[] {
        new BigInteger("86645e0320c0f9dc1a9b8456396cc105754df67a9829c21e13ab6ecf944cf68c", 16),
        new BigInteger("ea1721a578043d48f12738359b5eb5f0dac2242ec6128ee0ab6ff40c8fe0cae6", 16),
        ONE};

    // Q = lG
    BigInteger l =
        new BigInteger("b84d5cfab214fc3928864abb85f668a85b1006ca0147c78f22deb1dcc7e4a022", 16);
    BigInteger[] Q = new BigInteger[] {
        new BigInteger("61f6f7264f0a19f0debcca3efd079667a0112cc0b8be07a815b4c375e96ad3d1", 16),
        new BigInteger("3308c0016d776ed5aa9f021e43348b2e684b3b7a0f25dc9e4c8670b5d87cb705", 16),
        ONE};

    // Known answer for P+Q
    BigInteger[] kat1 = new BigInteger[] {
        new BigInteger("bc7adb05bca2460bbfeb4e0f88b61c384ea88ed3fd56017938ac2582513d4220", 16),
        new BigInteger("a640a43df2e9df39eec11445b7e3f7835b743ef1ac4a83cecb570a060b3f1c6c", 16)};
    ...
    BigInteger[] kat2 = new BigInteger[] {
        new BigInteger("c79d7f9100c14a70f0bb9bdce59654abf99e10d1ac5afc1a0f1b6bc650d6429b", 16),
        new BigInteger("6856814e47adce42bc0d7c3bef308c6c737c418ed093effb31e21f53c7735c97", 16)};
    ...
    BigInteger[] kat3 = new BigInteger[] {
        new BigInteger("97a82a834b9e6b50660ae30d43dac9b200276e8bcd2ed6a6593048de09276d1a", 16),
        new BigInteger("30a9590a01066d8ef54a910afcc8648dbc7400c01750af423ce95547f2154d56", 16)};
    ...
    BigInteger[] kat4 = new BigInteger[] {
        new BigInteger("6fd51be5cf3d6a6bcb62594bbe41ccf549b37d8fefff6e293a5bea0836efcfc6", 16),
        new BigInteger("9bc21a930137aa3814908974c431e4545a05dce61321253c337f3883129c42ca", 16)};
    ...
    String[] ecdsas = {"SHA1withECDSA", "SHA256withECDSA", "SHA384withECDSA", "SHA512withECDSA"};
    for (int s = 0; s < ecdsas.length; s++) {
      ...
      for (int i = 0; i < keyLengths.length; i++) {
        ...
        for (int n = 0; n < iter; n++) {
          ...
          ECPrivateKeySpec ecPrivSpec =
              new ECPrivateKeySpec(ecPrivKey.getS(), ecPrivKey.getParams());
          ...
          ECPublicKeySpec ecPubSpec = new ECPublicKeySpec(ecPubKey.getW(), ecPubKey.getParams());
          ...
          ecdsa.update(tmp);
          if (!ecdsa.verify(sig)) {
            throw new RuntimeException("Signature not verified: " + keyLengths[i]);
          }
          ...
          if (nssProv != null) {
            ...
            if (keyLengths[i] == 192 || keyLengths[i] == 224) {
              continue;
            }

            ECPrivateKey nssPrivKey = (ECPrivateKey) keyFab.generatePrivate(
                new PKCS8EncodedKeySpec(ecPrivKey.getEncoded()));
            ECPublicKey nssPubKey =
                (ECPublicKey) keyFab.generatePublic(new X509EncodedKeySpec(ecPubKey.getEncoded()));

            ecdsa = Signature.getInstance(ecdsas[s], nssProv);
            ecdsa.initVerify(nssPubKey);
            ecdsa.update(tmp);
            if (!ecdsa.verify(sig)) {
              throw new RuntimeException("Signature not verified 2: " + keyLengths[i]);
            }
            ...
            ecdsa.update(tmp);
            if (!ecdsa.verify(sig)) {
              throw new RuntimeException("Signature not verified 3: " + keyLengths[i]);
            }
          }
        }
