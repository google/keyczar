 ...
 *  2) Initialize session material which generates a AES session key and signing nonce. Returns RSA
 *     encrypted material.
 *  3) encrypt, which pulls the session material, encrypts the plaintext, using the session AES key.
 *  4) generate a signature based on the DSA key and attach the signature to the encrypted payload.
 *
...
public class SignedSessionEncrypter {
  ...
  private final AtomicReference<SessionMaterial> session = new AtomicReference<SessionMaterial>();
