...
/**
 * Unversioned Verifiers are used strictly to verify standard signatures (i.e. HMAC-SHA1, DSA-SHA1,
 * RSA-SHA1) with no key version information. Typically, UnversionedVerifiers will read sets of
 * public keys, although may also be instantiated with sets of symmetric or private keys.
 *
 *  Since UnversionedVerifiers verify standard signatures, they will try all keys in a set until one
 * verifies.
 *
 * {@link UnversionedSigner} objects should be used with symmetric or private key sets to generate
...
public class UnversionedVerifier extends Keyczar {
  ...
  /**
   * Initialize a new UnversionedVerifier with a KeyczarReader. The corresponding key set must have
   * a purpose of either {@link org.keyczar.enums.KeyPurpose#VERIFY} or
  ...
  /**
   * Initialize a new UnversionedVerifier with a key set location. This will attempt to read the
   * keys using a KeyczarFileReader. The corresponding key set must have a purpose of either
  ...
  /**
   * Verifies the standard signature on the data stored in the given ByteBuffer. This method will
   * try all keys until one of them verifies the signature, or else will return false.
   *
   * @param data The data to verify the signature on
  ...
  public boolean verify(String data, String signature) throws KeyczarException {
    try {
      return verify(data.getBytes(Keyczar.DEFAULT_ENCODING), Base64Coder.decodeWebSafe(signature));
