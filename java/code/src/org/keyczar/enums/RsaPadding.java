...
/**
 * Enumeration of available options for padding of plaintexts encrypted with RSA.
 *
 * Previously, only OAEP padding was supported. Some users had a hard need for PKCS#1 v1.5 padding
 * support, so it was added. However, OAEP is the preferred option because with PKCS#1 v1.5 RSA
 * vulnerable to some chosen ciphertext attacks.
 *
 * The default is OAEP. For maximum compatibility with other Keyczar implementations, the padding
 * field is omitted when writing keys with OAEP padding; it is only present when it is set to the
 * value "PKCS".
 *
