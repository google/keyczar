...
/**
 * Encrypters are used strictly to encrypt data. Typically, Encrypters will read sets of public
 * keys, although may also be instantiated with sets of symmetric keys.
 *
 * {@link Crypter} objects should be used with symmetric or private key sets to decrypt data.
 *
...
public class Encrypter extends Keyczar {
  ...
  private final StreamQueue<EncryptingStream> ENCRYPT_QUEUE = new StreamQueue<EncryptingStream>();

  /**
   * Initialize a new Encrypter with a KeyczarReader. The corresponding key set must have a purpose
   * of either {@link org.keyczar.enums.KeyPurpose#ENCRYPT} or
  ...
  /**
   * Initialize a new Encrypter with a key set location. This will attempt to read the keys using a
  ...
  /**
   * Returns the size of the ciphertext output that would result from encrypting an input of the
   * given length.
   *
  ...
  public int ciphertextSize(int inputLength) throws KeyczarException {
    ...
    int outputSize = HEADER_SIZE + cryptStream.maxOutputSize(inputLength) + signStream.digestSize();
   ...
   * @throws KeyczarException If there is a JCE exception or the key set does not contain a primary
   *         encrypting key.
   */
   ...
   * @throws KeyczarException If there is a JCE exception, the key set does not contain a primary
   *         encrypting key, or the output buffer is too small.
   */
