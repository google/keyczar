...
public interface KeyType {
  ...
  /**
   * Validates the specified parameters instance, throwing a {@link KeyczarException} if the
   * parameters cannot be used to generate a key and a warning string which may be displayed to
   * users if the parameters are valid but potentially problematic.
   *
  ...
  /**
   * Returns a new {@link KeyParameters} instance that overrides any unspecified parameters in the
   * provided parameters instance. The argument may be null, in which case all parameters will be
   * defaulted.
   */
   ...
   *
   * TODO(jmscheiner): This bit of misdirection isn't strictly necessary, but makes backwards
   * compatibility with the existing keys more straightforward.
   */
  ...
  public static class KeyTypeDeserializer implements JsonDeserializer<KeyType> {
     ...
     *
     * Custom {@link KeyType}s should be immutable singletons, Note that defining custom key types
     * is strongly discouraged for most applications.
     *
    ...
    @Override
    public KeyType deserialize(JsonElement json, Type type, JsonDeserializationContext context)
        throws JsonParseException {
      String keyName = json.getAsJsonPrimitive().getAsString();
      if (!typeMap.containsKey(keyName)) {
        throw new IllegalArgumentException(
            "Cannot deserialize " + keyName + " no such key has been registered.");
      }
